...
from datasets import dataset_read as Data
...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--crop_size', required=False, default=(256, 256), help='cropping size')
        parser.add_argument('--TTA', required=False, default=False, help='Test time augmentation')
        ...
        parser.add_argument('--pred_dir', required=False,
                            default=os.path.join(working_path, 'eval', DATA_NAME, 'test_map', 'NL'),
                            help='directory to output masks')
        parser.add_argument('--chkpt_path', required=False, default=os.path.join(working_path, 'checkpoints', DATA_NAME,
                                                                                 'SAM_CD_e23_OA75.20_F71.07_IoU61.86.pth'))
...
def stitch_pred(patch_list, size_stitch):
    ...
    for r in range(stitch_rows):
        ...
        if r == 0:
            stitched_img = stitched_r
        else:
            stitched_img = np.concatenate((stitched_img, stitched_r), axis=0)
...
def main():
    ...
    net = Net()
    net.to(torch.device('cuda', int(opt.dev_id)))
    net.load_state_dict(torch.load(opt.chkpt_path, map_location=torch.device('cuda', int(opt.dev_id))))
    net.eval()
...
def predict(net, opt):
    ...
    imgA_dir = os.path.join(opt.test_dir, 'image')
    data_list = os.listdir(imgA_dir)
    valid_list = []
    for it in data_list:
        if (it[-4:] == '.tif'): valid_list.append(it)

    for it in valid_list:
        imgA_path = os.path.join(imgA_dir, it)
        im_proj, im_geotrans, im_width, im_height, im_data = gdalTools.read_img(imgA_path)
        imgA = im_data.transpose(1,2,0)
        # imgA = io.imread(imgA_path)
        imgA = Data.normalize_image(imgA)

        with torch.no_grad():
            if imgA.shape[0] > opt.crop_size[0] or imgA.shape[1] > opt.crop_size[1]:
                imgA_crops = create_crops(imgA, opt.crop_size)
                crop_num = len(imgA_crops)
                print(it + ' (%d, %d, %d) cropped into %d patches.' % (
                imgA.shape[0], imgA.shape[1], imgA.shape[2], crop_num))
                preds = []
        ...
        pred_path = os.path.join(opt.pred_dir, it)
        gdalTools.write_img(pred_path, im_proj, im_geotrans, pred)
