...
class feature_Similarity(nn.Module):
    """input: x1, x2 field predictions and boundary predictions
       label_change: changed part
    """
    def __init__(self, reduction='mean'):
        super(feature_Similarity, self).__init__()
        self.loss_f = nn.CosineEmbeddingLoss(margin=0., reduction=reduction)

    def forward(self, x1, x2, mask_label):
        b,c,h,w = x1.size()
        x1 = F.sigmoid(x1)
        x2 = 1 - F.sigmoid(x2)
        x1 = x1.permute(0,2,3,1)
        ...
        target = torch.reshape(mask_label,[b*h*w])
        loss = target*(1-F.cosine_similarity(x1, x2))
