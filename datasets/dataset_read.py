...
def sliding_crop(imgs1, masks, contours, size):
    crop_imgs1 = []
    crop_imgs2 = []
    crop_labels = []
...
def rand_crop(img1, mask, contour, size):
    # print(img.shape)
    h = img1.shape[0]
    w = img1.shape[1]
    c_h = size[0]
...
def rand_flip(img1, mask, contour):
    r = random.random()
...
class RS(data.Dataset):
    def __init__(self, mode, random_crop=False, crop_nums=6, sliding_crop=False, crop_size=256, random_flip=True):
        ...
        if sliding_crop:
            data_A, mask, contour = sliding_crop(data_A, mask, contour, [self.crop_size, self.crop_size])
    ...
    def __getitem__(self, idx):
        ...
        if self.random_crop:
            data_A, mask, boundary = rand_crop(data_A, mask, boundary, [self.crop_size, self.crop_size])
        if self.random_flip:
            data_A,  mask, boundary = rand_flip(data_A, mask, boundary)
